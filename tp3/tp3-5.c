...
void codifica(mpz_t r, const char *str) {
	...
	for (i; i <= n; i++) {
		c = str[i];
		mpz_ui_pow_ui(pow, 256, i);
		mpz_mul_ui(aux, pow, c);
		mpz_add(r, r, aux);
	}
	mpz_clears(aux, pow, NULL);
}
...
void main() {
	...
	mpz_t C, M, r, n, e, d;
	mpz_inits(C, M, r, n, e, d, NULL);
	char str[500];
	char *strd;
	printf("Digite uma mensagem de até 500 caracteres: ");
	fgets(str, 500, stdin);
	printf("A mensagem e: %s \n", str);
	codifica(r, str);
	getchar();
	gmp_printf("A mensagem codificada e: %Zd\n", r);
	gera_chaves(n, e, d, rnd);
	getchar();
	criptografa(C, r, n, e);
	gmp_printf("A mensagem criptografada e: %Zd \n", C);
	descriptografa(M, C, n, d);
	getchar();
	gmp_printf("A mensagem descriptografada e: %Zd \n", M);
	strd = decodifica(r);
	getchar();
	printf("A mensagem decodificada e: %s \n", strd);
	mpz_clears(C, M, r, n, e, d, NULL);
	free(strd);
}